...
def test1(model):
    states = [[1] * model.width for i in range(model.height)]
    for agent in model.schedule.agents:
        state = np.array(agent.states)
        states += state
    states = np.array(states)
    states = states / model.num_agent
    return states[0][0]
def test2(model):
    states = [[1] * model.width for i in range(model.height)]
    for agent in model.schedule.agents:
...
class PoolSelectionModel(Model):
    ...
    def __init__(self, N, width, height):
        ...
        self.datacollector = DataCollector(
            model_reporters={"1": test1, '2': test2, '3': test3, '4': test4}
        )
...
if __name__ == '__main__':
    width = 2; height = 2
    model = PoolSelectionModel(N=20, width=width, height=height)
    for i in range(20):
        model.step()
